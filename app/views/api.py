...
def status():
    ...
    gps["lastUpdate"] = (
        (datetime.now() - gps_ser.pdopLastUpdate).total_seconds() if gps_ser.pdopLastUpdate is not None else -1
    )
    ret["gps"] = gps
    ...
    ret["components"] = component_health(tricap_manager, gps_ser, altimeter, os.path.ismount(MOUNT_POINT))
...
def sync_phone_time():
    ...
    if tricap_manager.state in (CAM_MANAGER_STATES.STARTED, CAM_MANAGER_STATES.COPYING):
        return jsonify({"msg": "Clock cannot be changed during capture or copying"}), 409

    try:
        epoch_ms, timezone_offset = validate_phone_time(request.get_json(silent=True))
    ...
    try:
        ...
        return jsonify({"msg": f"Could not set the device clock: {exc}"}), 500
...
def statistics():
    ...
    try:
        ...
        stats["captureInterval"] = float(tricap_manager.get_image_capture_interval())
        _logger.debug(stats)
        return stats
    except Exception:
...
def get_images(cam_idx):
    ...
    cam_session_dir = os.path.join(
        MOUNT_POINT,
        tricap_manager._copy_start_time.strftime("%Y_%m_%d"),
        tricap_manager._copy_start_time.strftime("%H_%M_%S"),
    )
    ...
    candidates = [p for p in Path(image_dir).iterdir() if p.is_file() and p.suffix.lower() in image_extensions]
...
def restart():
    ...
    _schedule_system_action(["systemctl", "--no-block", "restart", "tricap.service"])
...
def start():
    _logger.debug(f"Start req {tricap_manager.state}")
    ...
    if not tricap_manager.get_cameras_as_list():
        return jsonify({"msg": "No cameras connected. Connect at least one camera to start capture."}), 409

    if not tricap_manager.start_capturing():
        return jsonify({"msg": "Internal storage failed to mount. Capture not started."}), 409
    return {"success": True}


@api_bp.route("/api/stop_capture", methods=["POST"])
def stop():
    _logger.debug(f"Stop req {tricap_manager.state}")
...
def set_capture_interval():
    _logger.debug(f"set capture interval {tricap_manager.state}")
    ...
    miscSection["image_capture_interval"] = str(interval)
...
def verify_and_delete():
    _logger.debug(f"verify_and_delete {tricap_manager.state}")
    if tricap_manager.state in (CAM_MANAGER_STATES.STARTED, CAM_MANAGER_STATES.COPYING):
        return jsonify({"msg": "Not allowed in started or copying state"}), 400
    ...
    elif tricap_manager.external_ssd_device() is None and not os.path.ismount(MOUNT_POINT_SSD):
        return jsonify(
            {
...
def force_delete():
    _logger.debug(f"force_delete {tricap_manager.state}")
    if tricap_manager.state in (CAM_MANAGER_STATES.STARTED, CAM_MANAGER_STATES.COPYING):
        return jsonify({"msg": "Not allowed in started or copying state"}), 400
...
def netbird_connect():
    ...
    _logger.debug(f"netbird_connect {tricap_manager.state}")
    ...
    try:
        ...
        if result.returncode != 0:
            ...
            _logger.error(f"netbird up failed: {error_msg}")
            return jsonify({"msg": f"Failed to connect: {error_msg}", "success": False}), 500

        _logger.info("netbird up succeeded")
        ret = {"success": True, "msg": "Netbird connected successfully"}
        return jsonify(ret), 200

    except subprocess.TimeoutExpired:
        _logger.error("netbird connect command timed out")
        return jsonify({"msg": "Command timed out", "success": False}), 500
    except Exception as e:
        _logger.error(f"Error connecting netbird: {str(e)}")
        return jsonify({"msg": f"Error: {str(e)}", "success": False}), 500


@api_bp.route("/api/netbird_disconnect", methods=["POST"])
def netbird_disconnect():
    """Disconnect from netbird"""
    _logger.debug(f"netbird_disconnect {tricap_manager.state}")
    ...
    try:
        ...
        if result.returncode != 0:
            error_msg = result.stderr or result.stdout or "Unknown error"
            _logger.error(f"netbird down failed: {error_msg}")
            return jsonify({"msg": f"Failed to disconnect: {error_msg}", "success": False}), 500

        _logger.info("netbird down succeeded")
        ret = {"success": True, "msg": "Netbird disconnected successfully"}
        return jsonify(ret), 200
        ...
        _logger.error(f"Error disconnecting netbird: {str(e)}")
        return jsonify({"msg": f"Error: {str(e)}", "success": False}), 500
...
def netbird_status():
    ...
    _logger.debug(f"netbird_status {tricap_manager.state}")
    ...
    try:
        ...
        _logger.error(f"Error getting netbird status: {str(e)}")
        return jsonify(
            {
                "success": False,
                "connected": False,
    ...
    try:
        ...
        with _force_delete_lock:
            _force_delete_status.update(
                ...
                success=not errors,
                message=(
                    "Internal storage cleared." if not errors else "Internal storage could not be completely cleared."
                ),
                errors=errors[:50],
            )
    except Exception as exc:
