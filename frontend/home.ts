...
interface CaptureStatus {
  altimeter?: AltimeterStatus;
  camError?: boolean;
  cams: string[];
  components?: Partial<Record<"altimeter" | "cameras" | "gps" | "storage", ComponentStatus>>;
  gps?: GpsStatus;
  mode: string;
  progress?: string;
  wifiSignal?: number;
...
let latest: CaptureStatus | undefined;
let busy = false;
let lastStats: Statistics = {};
let lastStorageEstimate: StorageEstimate = {};

function sum(values: number[] | undefined): number {
  return values?.reduce((total, value) => total + Number(value || 0), 0) ?? 0;
}

function altitudeSettings(): AltitudeSettings {
...
function convertAltitude(value: number, from: "ft" | "m", to: "ft" | "m"): number {
  if (from === to) {
    return value;
  }
...
function chevronCount(error: number, band: number): number {
...
function cssColor(variable: string, styles: CSSStyleDeclaration): Rgb | undefined {
...
function chevronTone(nearVariable: string, farVariable: string, severity: number): string {
  ...
  const color = near.map((value, index) =>
    Math.round(value + ((far[index] ?? value) - value) * smooth).toString(16).padStart(2, "0"),
  );
  return `#${color.join("")}`;
}

function openGlance(): void {
...
function renderFlight(status: CaptureStatus, capturing: boolean): void {
  ...
  const sensorUnit: "ft" | "m" = altimeter.unit?.toLowerCase().startsWith("f") ? "ft" : "m";
...
function renderCameras(status: CaptureStatus, images: ImageCounts): void {
...
function flightTimeEstimate(stats: Statistics, estimate: StorageEstimate): { ready: boolean; text: string } {
  ...
  if (!(averageBytes > 0)) return { text: "Record images to estimate flight time", ready: false };
  if (!(cameraCount > 0)) return { text: "Connect cameras to estimate flight time", ready: false };
  if (!(interval > 0) || !(freeBytes > 0)) return { text: "Flight-time estimate unavailable", ready: false };
  return {
...
function renderStorage(stats: Statistics, estimate: StorageEstimate): void {
...
function render(status: CaptureStatus, images: ImageCounts): void {
  ...
  byId<HTMLButtonElement>("flightStop").disabled = busy;
  ...
  await singleFlight("home-status", async () => {
    try {
      const [status, images] = await Promise.all([
        getJson<CaptureStatus>("/api/status"),
        getJson<ImageCounts>("/api/images_captured").catch(() => ({})),
      ]);
      ...
      byId<HTMLButtonElement>("captureButton").disabled = true;
    }
  });
}
...
async function pollStorage(): Promise<void> {
  await singleFlight("home-storage", async () => {
    const [statsResult, estimateResult] = await Promise.allSettled([
      ...
      getJson<StorageEstimate>("/portal/storage_estimate"),
    ]);
...
async function toggleCapture(control: HTMLButtonElement): Promise<void> {
  ...
  busy = true;
  byId<HTMLButtonElement>("captureButton").disabled = true;
  byId<HTMLButtonElement>("flightStop").disabled = true;
  try {
...
function requestToggle(control: HTMLButtonElement): void {
  if (!latest || busy) return;
...
const captureButton = byId<HTMLButtonElement>("captureButton");
const flightStop = byId<HTMLButtonElement>("flightStop");
byId("host2").textContent = location.host || "control.skyseeker";
byId<HTMLButtonElement>("stopConfirmYes").addEventListener("click", () => {
  byId("stopConfirmModal").classList.remove("open");
  void toggleCapture(captureButton);
});
byId<HTMLButtonElement>("stopConfirmNo").addEventListener("click", () => byId("stopConfirmModal").classList.remove("open"));
