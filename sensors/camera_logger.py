"""As cameras are now subjects, we want a way to capture the notifications
emitted by a camera.

Trick is to have it happen in a separate thread, so that logging to disk does
not cause any delays to the operation of the camera.
"""
...
class cameraLoggingObserver(Observer, ThreadedLogger):
    """Observer for an abstract_camera camera, logs notification in separate
    thread."""

    def __init__(self, log_fp: str, subject_cameras=None):
        """Constructor, instantiate log file and handler with optional subject
        attachment."""
        ...
        getLogger().info('Rate logging file instantiated for camera at %s.', 
                         filename)
