...
class FlightLogTests(unittest.TestCase):
    ...
    def test_first_row_gets_friendly_header(self):
        flight_log.append_fix(self.tmp.name, 1,
                              datetime(2026, 9, 1, 10, 0, 0, 500000),
                              datetime(2026, 9, 1, 10, 0, 1),
                              25.7461234, 'S', 28.1881234, 'E', 1234.5, 0.9,
                              laser_altitude=None, use_latest_laser=False)
        rows = self._read()
        self.assertEqual(rows[0], list(flight_log.FLIGHT_LOG_HEADER))
        self.assertEqual(rows[0], ["Fix Quality", "GPS Time", "GPS Timestamp",
                                   "System Time", "System Timestamp",
                                   "Latitude", "Longitude", "GPS Altitude",
                                   "Laser Altitude", "HDOP"])
        gps_time = datetime(2026, 9, 1, 10, 0, 0, 500000)
        system_time = datetime(2026, 9, 1, 10, 0, 1)
        self.assertEqual(rows[1], ["1",
                                   "2026-09-01 10:00:00.500", str(gps_time.timestamp()),
                                   "2026-09-01 10:00:01.000", str(system_time.timestamp()),
                                   "-25.7461234", "28.1881234", "1234.50", "", "0.90"])

    def test_header_written_only_once(self):
        for _ in range(2):
            flight_log.append_fix(self.tmp.name, 1, datetime(2026, 9, 1), datetime(2026, 9, 1),
                                  1.0, 'N', 2.0, 'E', 0.0, 1.0, use_latest_laser=False)
        rows = self._read()
    ...
    def test_append_uses_latest_laser_reading(self):
        ...
        flight_log.append_fix(self.tmp.name, 2, datetime(2026, 9, 1), datetime(2026, 9, 1),
                              1.0, 'N', 2.0, 'E', 300.0, 1.1)
        self.assertEqual(self._read()[1][8], "52.25")

    def test_missing_values_are_blank_not_errors(self):
        row = flight_log.format_row(None, datetime(2026, 9, 1), datetime(2026, 9, 1),
                                    1.0, 'N', 2.0, 'E', None, '', None)
