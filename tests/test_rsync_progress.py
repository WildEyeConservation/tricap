...
class ParseLineTest(unittest.TestCase):
    def test_progress_line_gives_cumulative_bytes(self):
        self.assertEqual(
            parse_line("  1,234,567  12%   95.30MB/s    0:01:23 (xfr#42, to-chk=10/42)"), ("bytes", 1234567)
        )
        self.assertEqual(parse_line("987654  3%  1.00MB/s  0:00:01"), ("bytes", 987654))

    def test_transferred_file_line(self):
        self.assertEqual(
            parse_line(">f+++++++++|25165824|2024_01_01/DSC00001.ARW"), ("file", "2024_01_01/DSC00001.ARW")
        )
    ...
    def test_error_lines(self):
        for line in (
            ...
            'rsync: [sender] read errors mapping "x": Input/output error (5)',
        ):
            parsed = parse_line(line)
            assert parsed is not None
            self.assertEqual(parsed[0], "error")
