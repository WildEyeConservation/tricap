...
def usb_disk(children, serial: str | None = "S123"):
    return {
        "path": "/dev/sda",
        "type": "disk",
        "tran": "usb",
...
def part(
    ...
    size: str | int | None = "900",
    uuid: str | None = "1234-ABCD",
    partuuid: str | None = "p-1",
):
    return {"path": path, "type": "part", "fstype": fstype, "size": size, "uuid": uuid, "partuuid": partuuid}


def pick(blockdevices) -> Volume:
    volume = pick_volume(blockdevices)
...
class PickVolumeTest(unittest.TestCase):
    ...
    def test_prefers_largest_filesystem_over_leading_efi_partition(self):
        disk = usb_disk(
            [
                part("/dev/sda1", fstype="vfat", size="200", uuid="EFI-0001"),
                part("/dev/sda2", size="900", uuid="DATA-0002"),
            ]
        )
        self.assertEqual(pick([disk]).path, "/dev/sda2")

    def test_identity_fallback_chain(self):
        self.assertEqual(pick([usb_disk([part("/dev/sda1", uuid=None)])]).id, "p-1")
        self.assertEqual(pick([usb_disk([part("/dev/sda1", uuid=None, partuuid=None)])]).id, "S123")
        self.assertEqual(pick([usb_disk([part("/dev/sda1", uuid=None, partuuid=None)], serial=None)]).id, "/dev/sda1")
    ...
    def test_sizes_may_be_numbers_or_missing(self):
        ...
        self.assertEqual(pick([disk]).id, "A")
